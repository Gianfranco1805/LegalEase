...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    // Load recent docs
    const loadedDocs = JSON.parse(localStorage.getItem("mock_recent_docs") || "[]").map((d: any) => ({ ...d, folderId: d.folderId || null }));
    setRecentDocs(loadedDocs);
    
    // Load folders
    const storedFolders = localStorage.getItem("mock_folders");
    if (storedFolders) {
